  ...
  return (
    ...
    >
      ...
      <div className="flex w-full h-[calc(100vh-6rem)]" style={{ minHeight: '0' }}>
        ...
        {showSidebar && (
          <aside className="hidden lg:flex flex-col w-64 bg-black/70 backdrop-blur-sm border-r border-white/10 h-full p-4" style={{ minHeight: '0' }}>
            <nav className="flex-1 space-y-2">
              {navigationItems.map(item => (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`flex items-center px-4 py-2 rounded-lg text-white hover:bg-white/10 hover:text-blue-300 transition-colors ${location.pathname === item.path ? 'bg-white/10 font-bold' : ''}`}
                >
