...
interface User {
  ...
  name: string;
  email: string;
  role: 'student' | 'faculty' | 'service-provider' | 'admin';
...
const AdminTools: React.FC = () => {
  ...
  const [pendingApprovals, setPendingApprovals] = useState<PendingApproval[]>([
    {
      ...
      requestedBy: 'jane@university.edu',
      venue: 'Main Auditorium',
      date: '2024-02-15',
      details: 'Annual technology conference with 200 participants',
      status: 'pending'
    },
    {
      id: '2',
      ...
      requestedBy: 'council@university.edu',
      venue: 'Open Ground',
      date: '2024-02-20',
  ...
  const systemLogs = [
    {
      ...
      details: 'Created new booking for Main Auditorium'
    },
    {
      id: '2',
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user.id} className="border-t border-white/10 hover:bg-white/5 transition-colors">
                  ...
                  <td className="py-4 px-6">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium border ${user.role === 'admin' ? 'bg-purple-900/60 text-purple-200 border-purple-500/30' : user.role === 'faculty' ? 'bg-blue-900/60 text-blue-200 border-blue-500/30' : user.role === 'student' ? 'bg-green-900/60 text-green-200 border-green-500/30' : 'bg-orange-900/60 text-orange-200 border-orange-500/30'}`}>
  ...
  return (
    <Layout showSidebar={true} backgroundImage="/SD.jpg">
